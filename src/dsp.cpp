...
{
    int pad = n_fft / 2;
    signal.erase(signal.begin(), signal.begin() + pad); // remove 'pad' elements from the start

    auto it = signal.end() - pad;
    signal.erase(it, signal.end()); // remove 'pad' elements from the end
}

Eigen::Tensor3dXcf umxcpp::polar_to_complex(const Eigen::Tensor3dXf &magnitude, const Eigen::Tensor3dXf& phase) {
...
Eigen::Tensor3dXcf umxcpp::stft(const Eigen::MatrixXf &audio)
{
    ...
    auto stft_left = stft_inner(audio_left, window, FFT_WINDOW_SIZE, FFT_HOP_SIZE);
    auto stft_right = stft_inner(audio_right, window, FFT_WINDOW_SIZE, FFT_HOP_SIZE);
...
Eigen::MatrixXf umxcpp::istft(const Eigen::Tensor3dXcf &spec)
{
    ...
    std::vector<std::vector<std::complex<float>>> stft_left(rows, std::vector<std::complex<float>>(cols));
    std::vector<std::vector<std::complex<float>>> stft_right(rows, std::vector<std::complex<float>>(cols));
    ...
    audio.row(0) = Eigen::Map<Eigen::MatrixXf>(chn_left.data(), 1, chn_left.size());
    audio.row(1) = Eigen::Map<Eigen::MatrixXf>(chn_right.data(), 1, chn_right.size());
