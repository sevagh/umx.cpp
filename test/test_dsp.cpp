...
TEST(DSP_STFT, MagnitudePhaseCombineMono)
{
    ...
    Eigen::Tensor3dXf phase = spec.unaryExpr([](const std::complex<float> &c)
                                             { return std::arg(c); });
    ...
    for (size_t chan = 0; chan < spec.dimension(0); ++chan)
    {
        for (size_t i = 0; i < spec.dimension(1); ++i)
        {
            ...
            {
                EXPECT_NEAR(std::real(spec(chan, i, j)),
                            std::real(spec2(chan, i, j)), NEAR_TOLERANCE);
                EXPECT_NEAR(std::imag(spec(chan, i, j)),
                            std::imag(spec2(chan, i, j)), NEAR_TOLERANCE);
            }
        }
    }
