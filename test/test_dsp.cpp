...
TEST(DSP_STFT, MagnitudePhaseCombineMono) {
  ...
  Eigen::Tensor3dXf mag =
      spec.unaryExpr([](const std::complex<float> &c) { return std::abs(c); });

  Eigen::Tensor3dXf phase =
      spec.unaryExpr([](const std::complex<float> &c) { return std::arg(c); });
...
TEST(DSP_STFT, MagnitudePhaseCombineStereo) {
  Eigen::MatrixXf audio_in = umxcpp::load_audio("../test/data/gspi_stereo.wav");
